    ...
    public partial class XCProject : System.IDisposable
    {
        ...
        public void ApplyMod(XCMod mod)
        {
            ...
            if (buildSettings.ContainsKey("OTHER_LDFLAGS"))
            {
                ...
                if (otherLinkerFlags is string)
                {
                    list.Add(otherLinkerFlags.ToString());
                }
                else
                {
                    list.AddRange(((List<object>)otherLinkerFlags).Cast<string>().ToList());
                    
                }

                foreach (string linker in list)
                {
