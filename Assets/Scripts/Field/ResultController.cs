...
public class ResultController : MonoBehaviour {
	...
	public void SetTotalScore() {
		...
		if (transform.position.x > StageController.stageLengthMax) {
			...
			//残り風船ボーナス.
			if (GameController.GetNowBalloonNum () == 3) {
				remainBalloonScore = settings.scoreByBalloonGreen;
			} else if (GameController.GetNowBalloonNum () == 2) {
				remainBalloonScore = settings.scoreByBalloonYellow;
			}
		}
		float positionScore = distance / StageController.stageLengthMax * settings.scoreByMaxStage;

		//合算.
		addScore = remainTimeScore + remainBalloonScore + (int)positionScore;
	}
