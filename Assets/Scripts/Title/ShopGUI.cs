...
public class ShopGUI : MonoBehaviour {
	...
	private bool isInitBgService = false;
	...
	void RayFromInput(Vector2 point, int fingerID) {
		...
		if ( Physics.Raycast( ray, out hit ) && !isPusshing && isCanInput) {
			...
			if(hit.transform == btnShop7.transform) {
				...
				try {
					StoreInventory.BuyItem(IAPAssets.BALLOON7_PACK.ItemId);
				} catch (Exception e) {
			...
			if(hit.transform == btnShop40.transform) {
				...
				try {
					StoreInventory.BuyItem(IAPAssets.BALLOON40_PACK.ItemId);
				} catch (Exception e) {
					Debug.Log ("SOOMLA/UNITY " + e.Message);
				}
			}
			if(hit.transform == btnShop100.transform) {	
				...
				try {
					StoreInventory.BuyItem(IAPAssets.BALLOON100_PACK.ItemId);
				} catch (Exception e) {
					Debug.Log ("SOOMLA/UNITY " + e.Message);
				}
			}
