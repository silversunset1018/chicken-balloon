...
namespace Soomla.Example {
	...
	public static class ExampleLocalStoreInfo {
		...
		public static void UpdateBalances() {
			Debug.Log("  VirtualCurrencies Count  "+VirtualCurrencies);
			if (VirtualCurrencies != null && VirtualCurrencies.Count > 0) {
				CurrencyBalance = StoreInventory.GetItemBalance(VirtualCurrencies[0].ItemId);
				Debug.Log("  Balloon Count  "+CurrencyBalance);
			}
			...
			Debug.Log("  VirtualCurrencies Balloon Count  "+CurrencyBalance);
		}
