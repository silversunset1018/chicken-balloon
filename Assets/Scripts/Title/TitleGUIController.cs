...
public class TitleGUIController : MonoBehaviour {
	...
	private bool isVSChoose;	//VSボタンを押した.
	...
	void RayFromInput(Vector2 point, int fingerID) {
		...
		if ( Physics.Raycast( ray, out hit ) && !isPusshing && isCanInput) {
			...
			//対戦 / VS
			if(hit.transform == btnVS.transform) {
				btnVS.sprite = btnVSSprites[1];
				soundButton.Play();
				...
				if (Application.internetReachability != NetworkReachability.NotReachable){
					if (fresviiComp.GetAlreadySignup()) {
						isCanInput = false;
						//#FAS MatchMaking Start――――――

						bool success = fresviiComp.ShowFASMatchGui("");
						//#FAS MatchMaking End――――――
						if (!success) { 
							isCanInput = true;
							btnVS.sprite = btnVSSprites[0];
						}
					} else {
						isVSChoose = true;
			...
			if(hit.transform == btnForum.transform) {
				...
				if (Application.internetReachability != NetworkReachability.NotReachable){
					if (fresviiComp.GetAlreadySignup()) {
						isCanInput = false;
						//#FAS Forum Start――――――
						bool success = fresviiComp.ShowFASGui("");
						//#FAS Forum End――――――
						if (!success) {
							isCanInput = true;
							btnForum.sprite = btnForumSprites[0];
			...
			if(hit.transform == btnSignUpOK.transform) {
				...
				if (isVSChoose) {
					//#FAS MatchMake Start――――――
					bool success = fresviiComp.ShowFASMatchGui(userName);
					//#FAS MatchMake End――――――
					if (!success) {
						isCanInput = true;
						btnSignUpOK.sprite = btnSignUpOKSprites[0];
					}
				} else {
					//#FAS Forum Start――――――
					bool success = fresviiComp.ShowFASGui(userName);
					//#FAS Forum End――――――
					if (!success) {
						isCanInput = true;
						btnSignUpOK.sprite = btnSignUpOKSprites[0];
			...
			if(hit.transform == btnSignUpSkip.transform) {
				...
				if (isVSChoose) {
					...
					if (!success) {
						isCanInput = true;
						btnSignUpSkip.sprite = btnSignUpSkipSprites[0];
					}
				} else {
					//#FAS Forum Start――――――
					bool success = fresviiComp.ShowFASGui("");
